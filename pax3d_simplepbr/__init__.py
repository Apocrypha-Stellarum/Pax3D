...
class Pipeline:
    ...
    # Variables that require full tonemap rebuild when changed
    _BLOOM_REBUILD_VARS: ClassVar[list[str]] = [
        'enable_bloom',
        'bloom_levels',
    ]

    # Public instance variables (from simplepbr)
    ...
    _bloom_extract_quad: Any = field(init=False, default=None)
    _bloom_down_quads: list = field(init=False, default_factory=list)
    _bloom_up_quads: list = field(init=False, default_factory=list)
    ...
    def __post_init__(self, use_hardware_skinning: bool | None) -> None:
        ...
        # Validate tonemap operator
        if self.tonemap_operator not in _TONEMAP_OPERATORS:
            logging.warning(
                f'Unknown tonemap operator "{self.tonemap_operator}", falling back to "aces"'
            )
            self.tonemap_operator = 'aces'

        # Clamp bloom levels
        self.bloom_levels = max(2, min(8, self.bloom_levels))

        self._recompile_pbr()

        # Tonemapping (and bloom chain if enabled)
    ...
    @property
    def _tonemap_operator_index(self) -> int:
        return _TONEMAP_OPERATORS.get(self.tonemap_operator, 0)
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        ...
        # Pax3D: bloom uniform-only updates (no buffer rebuild needed)
        elif name == 'bloom_strength' and self._bloom_extract_quad is not None:
            self._bloom_extract_quad.set_shader_input('bloom_strength', self.bloom_strength)
        elif name == 'bloom_intensity' and self.enable_bloom:
            self._post_process_quad.set_shader_input('bloom_intensity', self.bloom_intensity)
        elif name == 'tonemap_operator':
            if value not in _TONEMAP_OPERATORS:
                logging.warning(
                    f'Unknown tonemap operator "{value}", falling back to "aces"'
                )
                super().__setattr__(name, 'aces')
            self._post_process_quad.set_shader_input(
                'tonemap_operator', self._tonemap_operator_index
            )
        ...
        # Pax3D: bloom structural changes require full rebuild
        if name in self._BLOOM_REBUILD_VARS and prev_value != value:
            resetup_tonemap()
    ...
    def _setup_tonemapping(self) -> None:
        ...
        # 1. Scene renders to RGBA16F (same as stock simplepbr)
        ...
        if self.enable_bloom:
            ...
            num_levels = max(2, min(8, self.bloom_levels))
            ...
            extract_quad.set_shader(shaderutils.make_shader(
                'bloom_extract', 'post.vert', 'bloom_extract.frag', bloom_defines
            ))
            ...
            # 3. Downsample chain (num_levels levels)
            ...
            for i in range(num_levels):
                ...
                quad.set_shader(shaderutils.make_shader(
                    'bloom_down', 'post.vert', 'bloom_downsample.frag', bloom_defines
                ))
                quad.set_shader_input('src_tex', down_textures[-1])
                # Texel size of the SOURCE texture (the one we're reading from)
            ...
            # 4. Upsample chain (num_levels levels, from smallest back to full res)
            up_tex = down_textures[-1]  # start from smallest mip
            for i in range(num_levels):
                src_idx = len(down_textures) - 2 - i  # work back up the chain
                # Divisor for this upsample level's output resolution
                ...
                quad.set_shader(shaderutils.make_shader(
                    'bloom_up', 'post.vert', 'bloom_upsample.frag', bloom_defines
                ))
                quad.set_shader_input('src_tex', down_textures[src_idx])
                quad.set_shader_input('bloom_accum_tex', up_tex)
                # Texel size of the source texture we're reading for the tent filter
                src_w = max(1, win_x // (2 ** (src_idx)))
                src_h = max(1, win_y // (2 ** (src_idx)))
                quad.set_shader_input('texel_size', p3d.LVecBase2(
                    1.0 / src_w, 1.0 / src_h
                ))
                # Per-mip tint (cycle through tints if more levels than tints)
