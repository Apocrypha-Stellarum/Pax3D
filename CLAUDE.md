...
| R6 | Engine hygiene (dead-path deletion, Vulkan watch) | **Windows 2+3+4 DONE:** DX9 excised (`cd37d67b93`), display backends excised (`8366907b14`), mobile-target machinery + DIRECTCAM excised (`baf541388a`, 2026-07-19) — −43k lines total, full gate green every time. No mobile code remains; remaining R6 = Vulkan watch only |
...
TLS pointer can never dangle when callers drop the return value; the
2026-07-28 async-screenshot shutdown fix — the screenshot fence callback
now honors the CompletionToken abandonment flag instead of running GL
against a GSG that is already being destroyed).
...
| ~~WINDOW 2: DX9 removal~~ | Deletion (R6) | **DONE 2026-07-17** — `cd37d67b93`, 65 files, −16,691 lines, gate green |
| ~~WINDOW 3: dead platform display backends~~ | Deletion (R6) | **DONE 2026-07-17** — `8366907b14`, 132 files, −18,546 lines (GLES/GLES2/EGL/WebGL/Android/iPhone/macOS backends + the DX9 flag machinery), gate green. **`--no-dx9` is no longer a valid makepanda option** |
| ~~WINDOW 4: mobile-target extraction~~ | Deletion (R6) | **DONE 2026-07-19** — `baf541388a` (+2 fixups), 72 files, −8,112 lines (android/iphone glue, Android cross-compile machinery, dist mobile deploy, DIRECTCAM), full gate identical both engines × both baselines. Wheel: `wheels_window4\` |
...
| ~~Core-profile combine-mode warning~~ | New C++ warning | **DONE 2026-07-18 (Session R mini-window, user-authorized)** — `e21bfc6ea7`, once-per-TextureAttrib glgsg warning in both default-shader paths; 1m22s incremental build, full gate identical both engines, wheel live in pax3d-env + archived `wheels_session_r\`. Day-one field value: the warning fired on a real silently-flattened combine state in sfb2's own boot |
...
| ~~GVAD handle-race stability fix~~ (restore `USE_DELETED_CHAIN=1` alongside mimalloc, makepanda.py:2327; restore both cycler stage guards to `#ifndef NDEBUG`; fix the `set_num_stages` interior delete, pipelineCyclerTrueImpl.cxx:334) — the reproducible chunk-mesher heap corruption (field crashes 2026-07-20 + 2026-07-23; **any cross-thread Geom construction vs destruction**; `workers=1` does NOT mitigate) | Build flag + 2 small C++ fixes | **DONE 2026-07-24 (GVAD stability window, user go-ahead)** — `1b3c76f0a6`, clean 12-min build, full acceptance green: every crashing repro row survives (deep soak 6.9M builds), gate both engines FAIL-sets-unchanged (Pax3D 82/7/129 · stock 80/7/131 incl. NEW permanent `test_gvad_churn` — FAILed on the Session-X wheel, PASSes on the fix + stock), testbeds + plan.py smoke green. Wheel live in pax3d-env AND system Python, archived `wheels_gvad\`. ER-011 mitigation no longer load-bearing. Optional follow-up stays available: poison-on-free diagnostic wheel |
...
| ~~Async-screenshot shutdown AV~~ (found Session AM, 2026-07-28): a readback still in flight when the process exits segfaulted it — ONE was enough, on both a real window and an offscreen buffer. Mechanism: the GL GSG's `_fences` deque holds `Fence{GLsync, CompletionToken}`; CompletionToken's documented contract is "destroyed prematurely == complete(false)", and the screenshot fence callback **ignored its success flag**, so premature destruction ran it anyway → `map_read_buffer`/`release_client_buffer` GL calls against a GSG already being destroyed. Hit upstream's own `save_async_screenshot()` at app exit too | Small C++ (GSG) | **DONE 2026-07-28 (Session AM build window, user-authorized)** — one branch in `glGraphicsStateGuardian_src.cxx` (`// PAX3D:`): on `success == false` do no GL, touch no GSG state, cancel the future (the sibling `extract_buffer_data` fence callback already branched this way). 42-second incremental build. Probe UNFIXED(139, 4 in flight) → FIXED(0, 6 in flight); every repro shape green (`probe_shutdown.py --mode pending`, `probe_mitigate.py --fix none`, real-window). NEW permanent gate row `engine_survives_inflight_exit` drives the RAW API on purpose so `FrameCapture.stop()`'s drain cannot mask a regression. Wheel live in pax3d-env AND system Python (both measured AV-ing before), archived `wheels_session_am\` |
...
| Pax3D venv | `C:\python\pax3d-env\` | Pax3D 1.11.0 (Session-AM wheel 2026-07-28 — async-screenshot shutdown fix; supersedes the bind-pin wheel, whose fixes it carries) + full game dep stack | **The game's default engine**; engine-build testing |
...
`wheels_bind_pin\`, `wheels_session_am\` (current);
...
catch-up merge** of upstream master (`25bffd98d2` — C++17 migration + 93
