...
| R5 | Atmospheric scattering, env-driven ambient, signature look | **Planetside slice landed opt-in (Session J, 2026-07-18):** aerial haze (`enable_atmosphere`), hemisphere/SH ambient (`set_hemisphere_ambient`), shadow texel snap (`shadow_texel_snap`) — all default-off = byte-identical, gated by 3 new paxtests. **Sessions K–L (2026-07-18): walkable-ship asset queue (master plan §4.8)** — specular-preserving glass (`set_glass`), double-sided lighting (`double_sided_lighting`), per-node ambient scale (`set_ambient_scale`) all landed opt-in, gated by test_glass/test_doublesided/test_ambient_scale. Remaining: openworld field tuning, orbital scattering, specular IBL (ship-dev-ranked "pure polish"), interior-collision design with the game dev (§4.8) |
