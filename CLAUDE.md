...
| **PBO round-robin framebuffer readback** (glReadPixels into 2–3 rotating pixel buffer objects, mapped 1–2 frames later — streaming sibling of `render_snapshot` for video capture/replay). Filed by Animal Crossfire 2026-07-28 (F9 H.264 recorder): sync `RTM_copy_ram` readback measured **+4.7 ms/frame floor @1600×900**, scales with area (4K ≈ 4×). Their contract: per-frame `bytes`/buffer view + w/h/format, 1–2-frame latency fine, BGRA preferred (x264 ingests without swizzle). Field notes on record: `RTM_triggered_copy_ram` measured ~10 ms/frame SLOWER than continuous copy on this stack (trigger path re-stalls rather than defers); Python's default 8 KB BufferedWriter GIL-convoys a render loop on Windows pipe writes (~700 writes per 5.8 MB frame). Serves planetside replay/photo tooling equally (concordance) | New C++ (GSG) + Python API | Queued 2026-07-28, LOW (their words: feature ships fine without it) — batch into the next scheduled build window |
