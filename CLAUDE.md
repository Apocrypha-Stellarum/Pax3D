...
| **GVAD handle-race stability fix** (restore `USE_DELETED_CHAIN=1` alongside mimalloc, makepanda.py:2327; restore both cycler stage guards to `#ifndef NDEBUG`; fix `set_num_stages` `delete[] &_single_data`, pipelineCyclerTrueImpl.cxx:334) — fixes the reproducible chunk-mesher heap corruption (field crashes 2026-07-20 + 2026-07-23; **any cross-thread Geom construction vs destruction**; `workers=1` does NOT mitigate). Full report `documents/CRASH_GVAD_HANDLE_RACE.md`; 60-s repro `tools/repro_gvad_race/`; acceptance = repro matrix survives + full gate. Optional follow-up: poison-on-free diagnostic wheel to name the exact racing line | Build flag + 2 small C++ fixes | **Queued 2026-07-23, user hold (“report only”)** — P0 for field stability; game-side mitigation filed as ER-011 (main-thread Geom construction) |
