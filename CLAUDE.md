...
| Core-profile combine-mode warning: when `do_issue_shader` selects `_default_shader` for a state whose TextureAttrib has an `M_combine` stage / `rgb_scale≠1` / >1 stage, emit a once-per-state glgsg warning (openworld round-4 P2: three features shipped silently inert; diagnosis in `OPENWORLD_FEEDBACK_RESPONSE_5.md`, probe `tools/paxtest/probe_texturestage.py`) | New C++ warning | Queued 2026-07-18 |
