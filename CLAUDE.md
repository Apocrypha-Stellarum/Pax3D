...
- **glTF alphaMode MASK only works in the compat profile (Session W,
  ...
  test_alpha_mask; master plan fact #17). Depth pass still casts the
  unmasked silhouette @modern — `exclude_from_shadows()` is the valve.
...
| ~~GPU morph path~~ (morph deltas + slider uniforms in the skinning vertex shader — HW skinning and morphs coexist) | GLSL + Python | **DONE 2026-07-20 (Session Z) — pure Python/GLSL, NO build window needed, runs on stock 1.10 too.** `set_gpu_morphs(np)`: per-vdata RGB32F delta texture (position AND normal deltas) + float32 `morph_index` column (both-baseline addressing, no gl_VertexID) + per-geom GPU_MORPHS variant; 52 targets addressable, ≤16 live, O(sliders) per-frame push. Gated: test_morph_gltf +5 checks, 12/12 all four configs, GPU-vs-CPU-valve rms 0.0000. Measured on hero_wren (worst-case 14,684×52): 8 faces ≈ 0.3 ms morph-attributable (**meets the ≤0.5 ms acceptance**) vs CPU valve 63.5 ms (~50×); 32 faces 2.42 ms; enable cost 1.17 s + 18.3 MB per face one-time. Fact #15 closed opt-in (default path byte-identical); fact #19 records the mechanism. Shadow silhouette unmorphed + variant-stacking = documented limits, field-evidence-gated. C++ promotion only if a profile ever demands |
