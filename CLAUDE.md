...
| ER (terrain lane) | Game-driven engine requests (`C:\python\sfb2\documents\ENGINE_REQUESTS\`): ER-001 splatting, ER-002 scatter, ER-003 data-texture contract | **Opened Session U (2026-07-19), all three accepted, interfaces pinned with the terrain dev.** ER-003 **IMPLEMENTED ENGINE-SIDE**: `data_texture()` stamp + `load_data_texture()` immune loader, gated by test_data_texture (13 checks, hostile `compressed-textures 1` live, both engines identical; gate now 56/6/77 @game). ER-001 next: `TERRAIN_SPLAT` per-subtree PBR variant, splat-map-driven, 2D texture arrays (3 samplers), v2 define seam for hex-tiling. ER-002: hardware instancing via the tree's existing `InstancedNode`/`p3d_InstanceMatrix` — pipeline INSTANCING define (pax_pbr.vert + shadow.vert) + `set_instanced()`, gated by a future test_instancing; no flatten fallback. See master plan §4.9 |
