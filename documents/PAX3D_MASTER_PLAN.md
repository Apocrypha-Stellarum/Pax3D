...
|---|---|---|
| R0 harness | `tools/paxtest/` — 10 test files, 5 pipelines, 2 GL baselines, analytic checks + instruments | **DONE**; gates everything (Session A; +2 tests Session G) |
...
| R2 directional sun + shadows | Pipeline-owned DirectionalLight, HPR-driven; shadows with world-space extent center; **hardened Session E**: world-unit bias, 3×3 PCF, no-cast API, skinned casters proven; **hardened Session G**: glTF caster darkening is a hard assertion, glTF caster+receiver test (angled sun), per-node `set_hardware_skinning()` opt-out | **Core done + hardened** (test_shadows 13+13, test_shadow_quality 9, test_shadows_gltf 6, test_skinning 12). Open: in-game validation — set `shadow_bias_world` (~0.5 IEU) first |
...
| Item | Home | Trigger |
|---|---|---|
| **Direction-gated lit-shadow failure (openworld P0 addendum, 2026-07-17 afternoon):** shadows die in a western low-sun cone (alt ≤~40°, az ~240) with a depth-error signature ~1/tan(alt); eastern mirror fine; deterministic `OW_SUN_OVERRIDE` repro in-app. Toy-scale sweep (`probe_azimuth_sweep.py`) is CLEAN — 4 azimuths × alt 34/45/60, both engines identical — so the trigger needs openworld scale and/or their exact daynight sun vectors. NOTE: this reopens the P0's attribution (their sun-arc-mapping reinterpretation of the 03:36/04:29 window is plausible, and fact #12 removed the pose-lucky counter-evidence); fact #11's worktree-hygiene lesson stands regardless | pipeline + paxtest (maybe their daynight.py) | **Next session, item 1** — plan in `HANDOVERS/handover_session_g_hardening_skinning.md` |
